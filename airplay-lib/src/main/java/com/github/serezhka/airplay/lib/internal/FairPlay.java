...
public class FairPlay {
    private static final Logger log = Logger.getLogger(FairPlay.class.getName());
    ...
    public void fairPlaySetup(InputStream request, OutputStream response) throws IOException {
        ...
        if (data[4] != 3) {
            log.severe("FairPlay version {} is not supported!" + data[4]);
            return;
        }
        if (data.length == 16) {
            int mode = data[14];
    ...
    /**
     * Java 8兼容的方法，用于读取InputStream中的所有字节
     */
